jcdfObject = function() {
    ...
    return {
		...
		openModalDialog : function(href, title, maxHeight, maxWidth, widthRate) {
			if (!widthRate) {
				widthRate = 0.98;
			}
			window.top.modalDialog(href, title, maxHeight, maxWidth, widthRate);
		},

		closeModalDialog:function() {
			window.top.closeModalDialog();
		},
   };
}
...
/*       error: function(XMLHttpRequest, textStatus, errorThrown) {
    	   if(XMLHttpRequest.readyState!=4 || XMLHttpRequest.status!=200){
        	   ...
        	   $$.showJcdfMessager('提示消息','响应异常，请检查服务器或网络是否正常','warning');
    	   }
	   },*/
       type: "POST",
...
//	window.location = '/ydcmp/login.html';
//}
...
function loadCombobox(fieldId,ddicType){
	
	$('#'+fieldId).combobox({
		url:ddicUrl+ddicType,
...
function loadSpecialCombobox(fieldId,ddicType){
	
	$('#'+fieldId).combobox({
		url:ddicUrl+ddicType,
		valueField : ddicType,
		textField : ddicType+'_name',
		filter : function(q, row) {
			var opts = $(this).combobox('options');
			return row[opts.textField].indexOf(q) > -1;
		},
	...
	function getAuthButtons(pageId,toolbarId,authActions, expellActions){
		...
		/*上面到的data的数据格式为："data":[{"group":"ems_mdm_lifnr_list","type":"B","level":0,"id":"ems_mdm_lifnr_exportEntities","url":"actions/mdm_lifnr/exportEntities.do","name":"导出供应商基础信息","menus":null,"resource2SubResource":null}]}*/
	...
	function generateButtons(buttons,pageId,authActions, expellActions){
		...
		for(var i=0;i<buttons.length;i++){
			if(buttons[i].type=="B"){
				...
				lis+= 
					"<a id='"+buttons[i].id+"' href='#' onclick=\"doAction('"+pageId+"','"+buttons[i].id+"','"+buttons[i].url+"')\" class='easyui-linkbutton' iconCls='icon-edit' plain='true'>"+buttons[i].name+"</a>\n";
			}
		}
