...
@Service
public class CIP_admin_auth_roleServiceImpl implements CIP_admin_auth_roleService {
	
	@Autowired
	private CIP_admin_auth_roleDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(CIP_admin_auth_roleVO vo, CIPRuntimeOperator operateInf){
		//TODO
		CIP_admin_auth_rolePO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_admin_auth_rolePO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(CIP_admin_auth_roleVO vo, CIPRuntimeOperator operateInf){
		CIP_admin_auth_rolePO po = vo.toPO();
		Object[] keys = po.getKeys();
		CIP_admin_auth_rolePO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public CIP_admin_auth_roleVO getData(Object[] keys ){
		CIP_admin_auth_rolePO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<CIP_admin_auth_roleData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<CIP_admin_auth_roleData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("cip_admin_auth_role");
			log.setOp_obj_id("");
			log.setOp_type("E");
