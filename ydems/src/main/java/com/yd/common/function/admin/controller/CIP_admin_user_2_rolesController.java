 ...
 */
@RestController
@RequestMapping(value = "/actions/admin_user_2_roles")
public class CIP_admin_user_2_rolesController {
	
	@Autowired
	private CIP_admin_user_2_rolesService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_admin_user_2_roles_addData";
	/**
	* 新增用户角色关联
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_admin_user_2_rolesVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_admin_user_2_roles_getData";
	/**
	* 查看用户角色关联
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_admin_user_2_rolesVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			CIP_admin_user_2_rolesVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "cip_admin_user_2_roles_deleteData";
	/**
	* 删除用户角色关联
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_admin_user_2_rolesVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_admin_user_2_roles_updateData";
	/**
	* 更新用户角色关联
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_admin_user_2_rolesVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_admin_user_2_roles_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<CIP_admin_user_2_rolesData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_admin_user_2_roles_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_admin_user_2_rolesMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_admin_user_2_rolesMapper.getExcelTitle(), entities);
				}
			}
