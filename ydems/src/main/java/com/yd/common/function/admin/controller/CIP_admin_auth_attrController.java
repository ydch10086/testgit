...
/**
 * <p>控制层实现类</p>
 * <p>Class: CIP_admin_auth_attrController</p>
 *
 * @since 2015-05-21 12:44:17
 */
@RestController
@RequestMapping(value = "/actions/admin_auth_attr")
public class CIP_admin_auth_attrController {
	
	@Autowired
	private CIP_admin_auth_attrService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "cip_admin_auth_attr_addData";
	/**
	* 新增权限对象属性信息
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody CIP_admin_auth_attrVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "cip_admin_auth_attr_getData";
	/**
	* 查看权限对象属性信息
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody CIP_admin_auth_attrVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			CIP_admin_auth_attrVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "cip_admin_auth_attr_deleteData";
	/**
	* 删除权限对象属性信息
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody CIP_admin_auth_attrVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "cip_admin_auth_attr_updateData";
	/**
	* 更新权限对象属性信息
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody CIP_admin_auth_attrVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "cip_admin_auth_attr_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<CIP_admin_auth_attrData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "cip_admin_auth_attr_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					CIP_admin_auth_attrMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, CIP_admin_auth_attrMapper.getExcelTitle(), entities);
				}
			}
