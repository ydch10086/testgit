...
import com.yd.ems.storage.dao.EMS_storage_logDao;
...
@Service
public class EMS_storage_logServiceImpl implements EMS_storage_logService {
	
	@Autowired
	private EMS_storage_logDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(EMS_storage_logVO vo, CIPRuntimeOperator operateInf){
		//TODO
		EMS_storage_logPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_storage_logPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(EMS_storage_logVO vo, CIPRuntimeOperator operateInf){
		EMS_storage_logPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_storage_logPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public EMS_storage_logVO getData(Object[] keys ){
		EMS_storage_logPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<EMS_storage_logData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<EMS_storage_logData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("em_storage_log");
			log.setOp_obj_id("");
			log.setOp_type("E");
