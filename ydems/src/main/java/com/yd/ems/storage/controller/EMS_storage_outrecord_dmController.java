...
import com.yd.ems.storage.data.mapper.EMS_storage_outrecord_dmMapper;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_storage_outrecord_dmController</p>
 *
 * @since 2015-05-29 02:42:14
 */
@RestController
@RequestMapping(value = "/actions/storage_outrecord_dm")
public class EMS_storage_outrecord_dmController {
	
	@Autowired
	private EMS_storage_outrecord_dmService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "ems_storage_outrecord_dm_addData";
	/**
	* 新增物料操作凭证
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody EMS_storage_outrecord_dmVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "ems_storage_outrecord_dm_getData";
	/**
	* 查看物料操作凭证
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_storage_outrecord_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_storage_outrecord_dmVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_storage_outrecord_dm_deleteData";
	/**
	* 删除物料操作凭证
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_storage_outrecord_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_storage_outrecord_dm_updateData";
	/**
	* 更新物料操作凭证
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_storage_outrecord_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_storage_outrecord_dm_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_storage_outrecord_dmData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_storage_outrecord_dm_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_storage_outrecord_dmMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_storage_outrecord_dmMapper.getExcelTitle(), entities);
				}
			}
