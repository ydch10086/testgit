...
import com.yd.ems.storage.dao.EMS_storage_objectDao;
...
@Service
public class EMS_storage_objectServiceImpl implements EMS_storage_objectService {
	
	@Autowired
	private EMS_storage_objectDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(EMS_storage_objectVO vo, CIPRuntimeOperator operateInf){
		//TODO
		EMS_storage_objectPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_storage_objectPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(EMS_storage_objectVO vo, CIPRuntimeOperator operateInf){
		EMS_storage_objectPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_storage_objectPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public EMS_storage_objectVO getData(Object[] keys ){
		EMS_storage_objectPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<EMS_storage_objectData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<EMS_storage_objectData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("em_storage_object");
			log.setOp_obj_id("");
			log.setOp_type("E");
