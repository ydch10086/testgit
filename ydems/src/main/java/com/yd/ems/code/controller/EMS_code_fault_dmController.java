...
import com.yd.ems.code.service.EMS_code_fault_dmService;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_code_fault_dmController</p>
 *
 * @since 2015-05-29 02:42:12
 */
@RestController
@RequestMapping(value = "/actions/code_fault_dm")
public class EMS_code_fault_dmController {
	
	@Autowired
	private EMS_code_fault_dmService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "ems_code_fault_dm_addData";
	/**
	* 新增故障类型
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody EMS_code_fault_dmVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "ems_code_fault_dm_getData";
	/**
	* 查看故障类型
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_code_fault_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_code_fault_dmVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_code_fault_dm_deleteData";
	/**
	* 删除故障类型
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_code_fault_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_code_fault_dm_updateData";
	/**
	* 更新故障类型
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_code_fault_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_code_fault_dm_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_code_fault_dmData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_code_fault_dm_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_code_fault_dmMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_code_fault_dmMapper.getExcelTitle(), entities);
				}
			}
