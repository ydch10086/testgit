...
import com.yd.ems.code.service.EMS_code_type_dmService;
import com.yd.ems.code.data.vo.EMS_code_type_dmVO;
import com.yd.ems.code.data.vo.EMS_parameterVO;
import com.yd.ems.code.data.key.EMS_code_type_dmKey;
import com.yd.ems.code.data.mapper.EMS_code_type_dmMapper;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_code_type_dmController</p>
 *
 * @since 2015-05-29 02:42:12
 */
@RestController
@RequestMapping(value = "/actions/code_type_dm")
public class EMS_code_type_dmController {
	
	@Autowired
	private EMS_code_type_dmService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "ems_code_type_dm_addData";
	/**
	* 新增对象类型
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody EMS_code_type_dmVO vo) {
	...
	public CIPResponseMsg addData(@RequestBody EMS_parameterVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			vo.setParameter(vo.getParameter() + " add");
			vo.setRemark("my returning");
			msg.data = vo;
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_ADDEQUIP_DATA = "ems_parameter_addEquipData";
	//添加设备数据
	@RequestMapping(value="/addEquipData")
	public CIPResponseMsg addEquipData(@RequestBody EMS_parameterVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			dataService.addEquipData(vo, CIPRuntime.getOperateSubject());
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
		} catch (CIPServiceException e) {
			CIPErrorCode error = e.getErrorCode();
	...
	private static final String C_ACTION_GET_DATA = "ems_code_type_dm_getData";
	/**
	* 查看对象类型
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_code_type_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_code_type_dmVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_code_type_dm_deleteData";
	/**
	* 删除对象类型
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_code_type_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_code_type_dm_updateData";
	/**
	* 更新对象类型
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_code_type_dmVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_code_type_dm_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_code_type_dmData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_code_type_dm_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_code_type_dmMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_code_type_dmMapper.getExcelTitle(), entities);
				}
			}
