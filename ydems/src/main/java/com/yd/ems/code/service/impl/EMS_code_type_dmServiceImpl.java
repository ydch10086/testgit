...
import com.yd.ems.code.dao.EMS_code_type_dmDao;
...
@Service
public class EMS_code_type_dmServiceImpl implements EMS_code_type_dmService {
	
	@Autowired
	private EMS_code_type_dmDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(EMS_code_type_dmVO vo, CIPRuntimeOperator operateInf){
		//TODO
		EMS_code_type_dmPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_code_type_dmPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(EMS_code_type_dmVO vo, CIPRuntimeOperator operateInf){
		EMS_code_type_dmPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_code_type_dmPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public EMS_code_type_dmVO getData(Object[] keys ){
		EMS_code_type_dmPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<EMS_code_type_dmData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<EMS_code_type_dmData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("em_code_type_dm");
			log.setOp_obj_id("");
			log.setOp_type("E");
