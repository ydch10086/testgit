...
import com.yd.ems.code.data.mapper.EMS_parameterMapper;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_parameterController</p>
 *
 * @since 2015-05-29 02:42:12
 */
@RestController
@RequestMapping(value = "/actions/parameter")
public class EMS_parameterController {
	
	@Autowired
	private EMS_parameterService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "ems_parameter_addData";
	/**
	* 新增对象类型
	*/
	...
	public CIPResponseMsg addEquipData(@RequestBody EMS_parameterVO vo,HttpServletRequest request) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			/*String typ_type = request.getParameter("typ_type");
			vo.setTyp_type(Integer.parseInt(typ_type));*/
	...
	private static final String C_ACTION_GET_DATA = "ems_parameter_getData";
	/**
	* 查看对象类型
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_parameterVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_parameterVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_parameter_deleteData";
	/**
	* 删除对象类型
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_parameterVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_parameter_updateData";
	/**
	* 更新对象类型
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_parameterVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_parameter_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_parameterData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_parameter_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_parameterMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_parameterMapper.getExcelTitle(), entities);
				}
			}
