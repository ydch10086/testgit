package com.yd.ems.check.controller;
...
import com.yd.ems.check.service.EMS_check_poi_logService;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_check_poi_logController</p>
 *
 * @since 2015-05-29 02:42:12
 */
@RestController
@RequestMapping(value = "/actions/check_poi_log")
public class EMS_check_poi_logController {
	
	@Autowired
	private EMS_check_poi_logService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "ems_check_poi_log_addData";
	/**
	* 新增设备日常点检日志
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody EMS_check_poi_logVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "ems_check_poi_log_getData";
	/**
	* 查看设备日常点检日志
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_check_poi_logVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_check_poi_logVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_check_poi_log_deleteData";
	/**
	* 删除设备日常点检日志
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_check_poi_logVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_check_poi_log_updateData";
	/**
	* 更新设备日常点检日志
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_check_poi_logVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_check_poi_log_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_check_poi_logData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_check_poi_log_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_check_poi_logMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_check_poi_logMapper.getExcelTitle(), entities);
				}
			}
