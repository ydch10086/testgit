...
import com.yd.ems.check.dao.EMS_check_poi_logDao;
...
@Service
public class EMS_check_poi_logServiceImpl implements EMS_check_poi_logService {
	
	@Autowired
	private EMS_check_poi_logDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(EMS_check_poi_logVO vo, CIPRuntimeOperator operateInf){
		//TODO
		EMS_check_poi_logPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_check_poi_logPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(EMS_check_poi_logVO vo, CIPRuntimeOperator operateInf){
		EMS_check_poi_logPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_check_poi_logPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public EMS_check_poi_logVO getData(Object[] keys ){
		EMS_check_poi_logPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<EMS_check_poi_logData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<EMS_check_poi_logData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("em_check_poi_log");
			log.setOp_obj_id("");
			log.setOp_type("E");
