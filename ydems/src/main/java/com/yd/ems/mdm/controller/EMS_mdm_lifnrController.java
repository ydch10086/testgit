...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_mdm_lifnrController</p>
 *
 * @since 2015-05-29 02:42:14
 */
@RestController
@RequestMapping(value = "/actions/mdm_lifnr")
public class EMS_mdm_lifnrController {
	
	@Autowired
	private EMS_mdm_lifnrService dataService  = null;
	
	public EMS_mdm_lifnrService getDataService() {
		return dataService;
	}

	public void setDataService(EMS_mdm_lifnrService dataService) {
		this.dataService = dataService;
	}

	private static final String C_ACTION_ADD_DATA = "ems_mdm_lifnr_addData";
	/**
	* 新增供应商基础信息
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody EMS_mdm_lifnrVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
//			CIPAuthLock lock = CIPRuntime.authManager.getAuthLock("test");
//			lock.setAuthValue("op_type", "A");
//			lock.setAuthValue("vend_type", vo.getSup_code());
//			lock.setAuthValue("create_user", CIPRuntime.getOperateSubject().getSubject_id());
//			boolean xAuth = CIPRuntime.authManager.checkBehavior(C_ACTION_ADD_DATA, lock);
	...
	private static final String C_ACTION_GET_DATA = "ems_mdm_lifnr_getData";
	/**
	* 查看供应商基础信息   通过主键查询
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_mdm_lifnrVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_mdm_lifnrVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_mdm_lifnr_deleteData";
	/**
	* 删除供应商基础信息
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_mdm_lifnrVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_mdm_lifnr_updateData";
	/**
	* 更新供应商基础信息
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_mdm_lifnrVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_mdm_lifnr_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_mdm_lifnrData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_mdm_lifnr_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_mdm_lifnrMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_mdm_lifnrMapper.getExcelTitle(), entities);
				}
			}
