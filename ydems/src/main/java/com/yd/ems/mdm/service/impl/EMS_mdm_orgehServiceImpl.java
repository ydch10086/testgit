...
import com.yd.ems.mdm.dao.EMS_mdm_orgehDao;
...
@Service
public class EMS_mdm_orgehServiceImpl implements EMS_mdm_orgehService {
	
	@Autowired
	private EMS_mdm_orgehDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(EMS_mdm_orgehVO vo, CIPRuntimeOperator operateInf){
		//TODO
		EMS_mdm_orgehPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_mdm_orgehPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(EMS_mdm_orgehVO vo, CIPRuntimeOperator operateInf){
		EMS_mdm_orgehPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_mdm_orgehPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public EMS_mdm_orgehVO getData(Object[] keys ){
		EMS_mdm_orgehPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<EMS_mdm_orgehData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<EMS_mdm_orgehData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("em_mdm_orgeh");
			log.setOp_obj_id("");
			log.setOp_type("E");
