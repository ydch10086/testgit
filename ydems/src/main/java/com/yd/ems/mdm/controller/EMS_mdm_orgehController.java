...
import com.yd.ems.mdm.service.EMS_mdm_orgehService;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_mdm_orgehController</p>
 *
 * @since 2015-05-29 02:42:14
 */
@RestController
@RequestMapping(value = "/actions/mdm_orgeh")
public class EMS_mdm_orgehController {
	
	@Autowired
	private EMS_mdm_orgehService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "ems_mdm_orgeh_addData";
	/**
	* 新增组织机构主数据
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody EMS_mdm_orgehVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "ems_mdm_orgeh_getData";
	/**
	* 查看组织机构主数据
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_mdm_orgehVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_mdm_orgehVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_mdm_orgeh_deleteData";
	/**
	* 删除组织机构主数据
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_mdm_orgehVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_mdm_orgeh_updateData";
	/**
	* 更新组织机构主数据
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_mdm_orgehVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_mdm_orgeh_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_mdm_orgehData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_mdm_orgeh_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_mdm_orgehMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_mdm_orgehMapper.getExcelTitle(), entities);
				}
			}
