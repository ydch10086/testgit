...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_mainten_repair_planController</p>
 *
 * @since 2015-05-29 02:42:12
 */
@RestController
@RequestMapping(value = "/actions/mainten_repair_plan")
public class EMS_mainten_repair_planController {
	
	@Autowired
	private EMS_mainten_repair_planService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "ems_mainten_repair_plan_addData";
	/**
	* 新增设备大修计划表
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody EMS_mainten_repair_planVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "ems_mainten_repair_plan_getData";
	/**
	* 查看设备大修计划表
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_mainten_repair_planVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_mainten_repair_planVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_mainten_repair_plan_deleteData";
	/**
	* 删除设备大修计划表
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_mainten_repair_planVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_mainten_repair_plan_updateData";
	/**
	* 更新设备大修计划表
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_mainten_repair_planVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_mainten_repair_plan_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_mainten_repair_planData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_mainten_repair_plan_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_mainten_repair_planMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_mainten_repair_planMapper.getExcelTitle(), entities);
				}
			}
