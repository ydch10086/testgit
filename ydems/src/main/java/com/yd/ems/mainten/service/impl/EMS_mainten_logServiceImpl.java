...
import com.yd.ems.mainten.dao.EMS_mainten_logDao;
...
@Service
public class EMS_mainten_logServiceImpl implements EMS_mainten_logService {
	
	@Autowired
	private EMS_mainten_logDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(EMS_mainten_logVO vo, CIPRuntimeOperator operateInf){
		//TODO
		EMS_mainten_logPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_mainten_logPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(EMS_mainten_logVO vo, CIPRuntimeOperator operateInf){
		EMS_mainten_logPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_mainten_logPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public EMS_mainten_logVO getData(Object[] keys ){
		EMS_mainten_logPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<EMS_mainten_logData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<EMS_mainten_logData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("em_mainten_log");
			log.setOp_obj_id("");
			log.setOp_type("E");
