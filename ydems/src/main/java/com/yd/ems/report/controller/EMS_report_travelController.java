package com.yd.ems.report.controller;
...
/**
 * <p>控制层实现类</p>
 * <p>Class: EMS_report_travelController</p>
 *
 * @since 2015-05-29 02:42:14
 */
@RestController
@RequestMapping(value = "/actions/report_travel")
public class EMS_report_travelController {
	
	@Autowired
	private EMS_report_travelService dataService  = null;
	
	private static final String C_ACTION_ADD_DATA = "ems_report_travel_addData";
	/**
	* 新增人员出差报告
	*/
	@RequestMapping(value="/addData")
	public CIPResponseMsg addData(@RequestBody EMS_report_travelVO vo) {
	...
	private static final String C_ACTION_GET_DATA = "ems_report_travel_getData";
	/**
	* 查看人员出差报告
	*/
	@RequestMapping(value = "/getData")
	public CIPResponseMsg getData(@RequestBody EMS_report_travelVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
			EMS_report_travelVO vo0 = dataService.getData(keys);
			msg.errorCode = CIPErrorCode.CALL_SUCCESS.code;
			msg.msg = CIPErrorCode.CALL_SUCCESS.name;
			msg.data = vo0;
		} catch (CIPServiceException e) {
	...
	private static final String C_ACTION_DELETE_DATA = "ems_report_travel_deleteData";
	/**
	* 删除人员出差报告
	*/
	@RequestMapping(value = "/deleteData")
	public CIPResponseMsg deleteData(@RequestBody EMS_report_travelVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
			Object[] keys = vo.getKeys();
	...
	private static final String C_ACTION_UPDATE_DATA = "ems_report_travel_updateData";
	/**
	* 更新人员出差报告
	*/
	@RequestMapping(value = "/updateData")
	public CIPResponseMsg updateData(@RequestBody EMS_report_travelVO vo) {
		CIPResponseMsg msg = new CIPResponseMsg();
		try {
	...
	private static final String C_ACTION_SEARCH_DATA = "ems_report_travel_searchData";
	...
	*/
	...
	public CIPResponseQueryMsg searchData(CIPReqParameter parameter) {
		...
		try {
			...
			List<EMS_report_travelData> vos = dataService.searchData(page,conditions);
	...
	private static final String C_ACTION_EXPORT_EXCEL = "ems_report_travel_exportEntities";
	...
	public void exportEntities(CIPReqParameter parameter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			SXSSFWorkbook wb = ExcelSheetParser.createWorkBook(sheetName,
					EMS_report_travelMapper.getExcelTitle(), entities);
			...
			if (total > rows) {
				...
				for (int i = 2; i <= count; i++) {
					...
					ExcelSheetParser.appendWorkBook(wb, sheetName, EMS_report_travelMapper.getExcelTitle(), entities);
				}
			}
