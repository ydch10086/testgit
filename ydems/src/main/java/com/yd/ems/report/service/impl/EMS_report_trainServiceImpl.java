...
import com.yd.ems.report.service.EMS_report_trainService;
...
@Service
public class EMS_report_trainServiceImpl implements EMS_report_trainService {
	
	@Autowired
	private EMS_report_trainDao dataDao = null;
	@Autowired
	 ...
	 */
	@Override
	public void updateData(EMS_report_trainVO vo, CIPRuntimeOperator operateInf){
		//TODO
		EMS_report_trainPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_report_trainPO po0 = dataDao.getSingle(keys);
		if( po0 == null ){
	 ...
	 */
	@Override
	public void addData(EMS_report_trainVO vo, CIPRuntimeOperator operateInf){
		EMS_report_trainPO po = vo.toPO();
		Object[] keys = po.getKeys();
		EMS_report_trainPO po0 = dataDao.getSingle(keys);
		if( po0!= null ){
	...
	public EMS_report_trainVO getData(Object[] keys ){
		EMS_report_trainPO  po = dataDao.getSingle(keys);
		if(po == null)
			throw new CIPServiceException(CIPErrorCode.ERROR_RECORD_NOT_EXISTS);
	...
	public List<EMS_report_trainData> searchData(CIPPageInfo page, CIPReqCondition[] conditions){
		...
		List<EMS_report_trainData> datas = dataDao.searchData(page, conditions);
		return datas;
	}
	...
	public List<Map<String, Object>> exportEntities(CIPPageInfo page,CIPReqCondition[] conditions,
		...
		if(xFirst){
			...
			log.setOp_table_id("em_report_train");
			log.setOp_obj_id("");
			log.setOp_type("E");
